...
function site_footer_render(PDO $pdo): void
{
    ...
    $useHtmlTiles = function_exists('current_lang') && current_lang($pdo) !== 'tr' && function_exists('shop_footer_info_tiles');

    if (!$showImages && !$showLegal && !$useHtmlTiles) {
        return;
    }
    ...
    if ($useHtmlTiles) {
        echo '<div class="site-footer__tiles">';
        foreach (shop_footer_info_tiles() as $tile) {
            $tone = htmlspecialchars((string) ($tile['tone'] ?? 'slate'), ENT_QUOTES, 'UTF-8');
            echo '<div class="site-footer__tile site-footer__tile--' . $tone . '">';
            echo '<div class="site-footer__tile-h"><i class="fas ' . htmlspecialchars((string) ($tile['icon'] ?? 'fa-circle-check'), ENT_QUOTES, 'UTF-8') . '" aria-hidden="true"></i> ' . htmlspecialchars((string) ($tile['title'] ?? ''), ENT_QUOTES, 'UTF-8') . '</div>';
            echo '<p>' . htmlspecialchars((string) ($tile['text'] ?? ''), ENT_QUOTES, 'UTF-8') . '</p>';
            echo '</div>';
        }
        echo '</div>';
    } elseif ($showImages) {
