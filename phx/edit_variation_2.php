...
$stmt = $pdo->prepare('SELECT * FROM product_variations_2 WHERE variation_id = ?');
...
$stmt = $pdo->prepare('SELECT * FROM product_variants_2 WHERE variation_id = ?');
$stmt->execute([$variation_id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("INSERT INTO product_variants_2 (variation_id, variant_name) VALUES (?, ?)");
    $stmt->execute([$variation_id, $variant_name]);

    $_SESSION['message'] = 'Varyant başarıyla eklendi!';
    header('Location: edit_variation_2.php?variation_id=' . rawurlencode((string)$variation_id));
    exit();
}

$page_title = 'Varyant ekle (2) — ' . htmlspecialchars($variation['variation_name']);
...
<div class="container-fluid px-0">
        ...
        <ul class="list-group mb-4">
            <?php foreach ($variants as $variant): ?>
                <li class="list-group-item d-flex justify-content-between align-items-center flex-wrap gap-2">
                    ...
                    <span>
                        <a href="edit_variant_2.php?variant_id=<?= (int)$variant['variant_id'] ?>" class="btn btn-outline-secondary btn-sm">Düzenle</a>
                        <a href="delete_variant_2.php?variant_id=<?= (int)$variant['variant_id'] ?>&variation_id=<?= (int)$variation_id ?>" class="btn btn-outline-danger btn-sm" onclick="return confirm('Bu varyantı silmek istediğinize emin misiniz?');">Sil</a>
                    </span>
                </li>
            <?php endforeach; ?>
        </ul>
    ...
    <a href="add_variation_2.php" class="btn btn-outline-secondary">Varyasyon 2 listesine dön</a>
</div>
